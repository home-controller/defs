...
#define InOnlyPins                                                             \
  = 2 // ATmega328P chips have 2 analog input only pins namely A6 & A7. Some
      // older boards do not bring them out to pins but most do.
#endif
...
#define _pwmPins                                                               \
  { 3, 5, 6, 9, 10, 11 } // Pro Micro don't have pins 11,12,13 brocken out.

// timer interferes with 5 & 6
#define InOnlyPins                                                             \
  = 0 // ATmega328P chips have 2 analog input only pins namely A6 & A7. Some
      // older boards do not bring them out to pins but most do.
#endif
...
#define board_type Pro_Micro_board
#else
#define _pwmPins                                                               \
  {                                                                            \
    0xff                                                                       \
  } // using the max value with array size of 1 only to signify that there are
    // no pwm pins.
...
// The type of Input the switch is on. e.g. Local mcu GPIO pin, 1wire, IO
// extender, shift register etc.
#define ct_pin 0 // ct stands for controller type
#define ct_1wire 1
#define ct_I2C                                                                 \
  2 // this should probably be more specific as in a type of i2c IO expander.
#define ct_SPI 3
#define ct_array 4
#define ioLocalPin 0
...
byte cpBitsMsk8(byte a, byte b,
                byte msk); // set bit in a matched by msk to value(s) from b
